...
const BOX_TYPES = {
  ...
  reverse:        { zone: "any", description: "Reverse the order of an array. Non-arrays pass through unchanged.",
                    inlets: [{ name: "in", type: "array", description: "Input array" }],
                    outlets: [{ name: "out", type: "array", description: "Reversed copy" }] },
  ...
  assign:         { zone: "router", description: "Stable voice allocation. Distributes a notes array (e.g. from `held`) across phones with minimum movement: phones already on a still-held note stay put; only over-target phones move when the array changes. New phones audibly join; departing phones cause a reshuffle. If more notes than phones, the oldest notes are dropped.",
                    inlets: [{ name: "notes", type: "array", description: "Array of notes/freqs to distribute across phones (triggers allocation)" },
                             { name: "amps", type: "array", description: "Optional parallel amplitudes (index-aligned to notes); makes each per-phone send a [freq, amp] pair for spectral resynthesis" }],
                    outlets: [{ name: "out", type: "passthrough", description: "Per-phone assigned note, or [freq, amp] pair when amps connected" }] },
  ...
  "fftPeaks~":    { zone: "ctrl", description: "Spectral peak analyser (a→k). On a trigger, finds the N loudest spectral peaks of the input audio and outputs them as two parallel arrays, sorted loudest-first. Pairs with `assign` for distributed resynthesis.", args: "[maxPeaks] [minHz]", example: "fftPeaks~ 33 100",
                    inlets: [{ name: "in", type: "audio", description: "Audio to analyse (e.g. from adc~)" },
                             { name: "trigger", type: "event", description: "Scan now (e.g. from metro)" },
                             { name: "max", type: "number", description: "Live cap on peaks emitted (e.g. from `clients` — keep ≤ phone count); overrides the arg" },
                             { name: "minHz", type: "number", description: "Ignore peaks below this frequency (Hz) — filters DC/hum/room rumble; 0 = off. Overrides the arg" }],
                    outlets: [{ name: "freqs", type: "array", description: "Peak frequencies (Hz), loudest first" },
                              { name: "amps", type: "array", description: "Peak amplitudes (linear), index-aligned to freqs" }] },
  ...
  "karplus-strong~": { zone: "any", description: "Karplus-Strong plucked string synthesis.",
                    inlets: [
                      ...
                      { name: "amplitude", type: "number", description: "Output level (0-1)" }],
                    outlets: [{ name: "out", type: "audio", description: "Audio output" }] },
  "ewing~":       { zone: "synth", description: "Ewing's Tree Frog (Litoria ewingii). Autonomous frog — picks its own personality at init (formant pitch, pulse rate, call template) and chatters on its own schedule. Hold engages: the grain stream that was exciting the species formant smoothly shifts to excite a formant at the user-controlled pitch — no separate held tone, just the ongoing ring continued at a different pitch. Arg: portamento seconds — governs both chatter→singing morph AND note-to-note pitch glides in singing (default 1.0).",
                    args: "[glide_sec]", example: "ewing~ 1.0",
                    inlets: [
                      ...
                      { name: "active", type: "number", description: "On/off gate. Calls wind up when high and wind down when low over glideSec × 6 (default 6s)." }],
                    outlets: [{ name: "out", type: "audio", description: "Audio output" }] },
  "frog~":        { zone: "synth", description: "Single-frog voice. Pulse train through formants with biological micro-modulation. Trigger fires a burst; hold morphs to a sustained pitched tone for chord work. Arg: pitch portamento seconds (default 0.15).",
                    args: "[glide_sec]", example: "frog~ 0.15",
                    inlets: [
